...
Hadley Dixon, CIS 211, 04-29-2022
...
class Board(object):
    ...
    def __init__(self):
        ...
        for col_i in range(len(self.tiles[0])):
            ...
            for row_i in range(len(self.tiles)):
                main_index = self.tiles[row_i]
                sub_index = main_index[col_i]
                item_lst.append(sub_index)
            self.groups.append(item_lst)
    ...
    # notification of a new event to communicate that information
    ...
    def naked_single(self) -> bool:
        ...
        change_naked = False
        for group in self.groups:
            # Get set of symbols used in that group
            set_of_symbols = set()
            for tile in group:
                set_of_symbols.add(tile.value)
            # Eliminate those symbols from the candidate sets of all
            # the unknown tiles in the group
            for tile in group:
                if tile.remove_candidates(set_of_symbols):
                    change_naked = True
        return change_naked
    ...
    def is_complete(self) -> bool:
        ...
        Loops through the tiles to determine whether
        all the tiles have values from CHOICES
        """
        for group in self.groups:
            for tile in group:
    ...
    def solve(self) -> bool:
        ...
        True means board was solved
        False means board was not solved, and it cannot be solved,
        because of some wrong guess made early
        """
        self.propagate()
        if self.is_complete():
            return True
        ...
        save_board = self.as_list()
        tester = self.min_choice_tile()
        for candidate in tester.candidates:
            tester.set_value(candidate)
            if self.solve():
                return True
            else:
                self.set_tiles(save_board)
        return False
